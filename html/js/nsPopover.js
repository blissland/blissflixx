    ...
    return {
      ...
      link: function(scope, elm, attrs) {
        ...
        var displayer_ = {
          ...
          display: function(popover, delay, e, target) {
            ...
            displayer_.id_ = $timeout(function() {
              ...
              // |elm|.
              var elmRect = getBoundingClientRect(target ? target : elm[0]);
        ...
        if (options.angularEvent) {
          $rootScope.$on(options.angularEvent, function(evt, target) {
            hider_.cancel();
            displayer_.display($popover, options.popupDelay, null, target);
          });
          $rootScope.$on("hidePopover", function() {
            hider_.hide($popover, 0);
          });
        } else {
